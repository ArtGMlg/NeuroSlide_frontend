...
<template>
  <div class="slide" :style="{ transform: `scale(${scaleFactor})` }">
    <div class="Style1SingleText content"
      style="width: 1920px; height: 1080px; position: relative;" :style="{ backgroundColor: color || '#E5DFC8' }">
      <div class="Line1"
      ...
      <div contenteditable @input="(v) => emit('inputText', (v.target as HTMLElement).innerHTML || 'Text')"
        style="width: 1514px; left: 203px; top: 302px; position: absolute; color: black; font-size: 48px; font-weight: 400; line-height: 64.80px; word-wrap: break-word"
        :style="{ fontFamily: font || 'Lora' }"
      >
        {{ text }}</div>
